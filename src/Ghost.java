...
public class Ghost extends Thread {
	...
	public Ghost(int y, int x, Matrix my_matrix, Mypanel p) throws IOException {
		...
		this.x = x;
		this.y = y;
		this.right_image = new ImageIcon("ghost_right.png").getImage();
		...
		My_matrix.update_ghost_place(x, y);
		this.setName("ghost");
	}
	...
	public int findNextAction(int x, int y, int depth, int x0, int y0) {
		...
		//visited matrix to mark visited nodes to not revisit them
		//x0 and y0 is used to pass as initial node (from which we start the search) to
		//all minScore functions to subtract from their current search position and 
		//find relative position. if x0=100,y0=140,  and current search node is x=120 y=y0=140
		// then (120-100)/20= +1 and so we mark 1 cell to the left of the middle cell of visited
		// matrix as 1(visited)		
	...
	public int minScore(int x, int y, int depth, int visited[][], int x0, int y0, int d0) {
		//d0+ (+1 +2 +3 +4 +...+-1 -2 -3) we add d0 so that visited matrix indices wont be negative
		int dx = (x - x0) / 20 + d0; 
		int dy = (y - y0) / 20 + d0;
